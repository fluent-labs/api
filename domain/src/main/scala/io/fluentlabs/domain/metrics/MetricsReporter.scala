...
import io.fluentlabs.domain.metrics.label.ElasticsearchMethod.ElasticsearchMethod
import io.fluentlabs.domain.metrics.label.RequestPath.RequestPath
import io.fluentlabs.domain.metrics.label.WebsterDictionary.WebsterDictionary
...
class MetricsReporter @Inject() (holder: MetricHolder, config: Configuration) {
  ...
  def reportUnauthenticatedRequest(path: RequestPath): Unit =
    holder.report(Metric.UNAUTHENTICATED_REQUEST, path.toString)
  def reportBadRequestToken(path: RequestPath): Unit =
    holder.report(Metric.BAD_REQUEST_TOKEN, path.toString)
