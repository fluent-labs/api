...
def test_chinese():
    ...
    assert words == [Word(token='测试', tag='动词', lemma='测试'),
...
def test_english():
    ...
    assert words == [Word(token='The', tag='DET', lemma='the'),
...
def test_spanish():
    words = tag("SPANISH", "La prueba ha pasado porque estas palabras fueron devueltas.")
    assert words == [Word(token='La', tag='DET', lemma='La'),
